...
function setCheckoutDisabled(disabled) {
  payCashBtn?.setAttribute("disabled", disabled ? "true" : "");
  payUpiBtn?.setAttribute("disabled", disabled ? "true" : "");
  payCashBtn?.classList.toggle("opacity-50", disabled);
  payUpiBtn?.classList.toggle("opacity-50", disabled);
}
const cart = new Cart();
